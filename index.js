...
const is31Flavors = (flavors => (flavors === 31 ? true : false));
...
const addFlavor = nFlavors => {
    nFlavors.unshift("Rainbow Sherbert")
    return nFlavors
}
...
const getFlavorByIndex = fetching => {
    return fetching[5]
}
...
const removeFlavorByName = target => {
    target.indexOf("vanilla")
    target.splice(-1, 1)
    return target
}
...
const copy = (originalFlavors, copyFlavors) => {
    copyFlavors = []
}
...
const filterByWord = (flavor) => {

    if (originalFlavors.includes(flavor)) {
        for (flavors of originalFlavors)
            console.log([flavors])
    }
